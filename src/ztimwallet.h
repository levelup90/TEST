...
#ifndef TIMcoin_ZTIMWALLET_H
#define TIMcoin_ZTIMWALLET_H
...
public:
    CzTIMWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZTIM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZTIM(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //TIMcoin_ZTIMWALLET_H
