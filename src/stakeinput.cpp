...
// The zTIM block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zTIM is a hash of the serial
...
{
    //Create an output returning the zTIM that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZTIMOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zTIM output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zTIM", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZTIMOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zTIM output", __func__);
...
{
    CzTIMTracker* ztimTracker = pwallet->ztimTracker.get();
    CMintMeta meta;
    if (!ztimTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    ztimTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a TIM stake is the outpoint
