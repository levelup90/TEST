...
CzTIMWallet::CzTIMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing ztim seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTIMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTIMSeed_deprecated()) {
                    LogPrintf("%s: Updated zTIM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ztim seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ztim wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzTIMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTIMCount(nCountLastUsed);
    else if (!walletdb.ReadZTIMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTIMWallet::Lock()
{
    seedMaster = 0;
}

void CzTIMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTIMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTIM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTIMWallet::LoadMintPoolFromDB()
{
...
void CzTIMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTIMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTIMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->ztimTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTIMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTIM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to ztimTracker which also adds to database
    pwalletMain->ztimTracker->Add(dMint, true);
...
void CzTIMWallet::SeedToZTIM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTIMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTIMWallet::GenerateDeterministicZTIM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTIMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTIMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
