...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered timcoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 TIM.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal timcoin:URI (timcoin:XyZ...?message=example)
            {
...
static void NotifyzTIMReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzTIMReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzTIMReset.connect(boost::bind(NotifyzTIMReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzTIMReset.disconnect(boost::bind(NotifyzTIMReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->ztimTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
