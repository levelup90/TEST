...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TIM);
    ...
    if (!settings.contains("nAnonymizeTIMcoinAmount"))
        settings.setValue("nAnonymizeTIMcoinAmount", 1000);

    nAnonymizeTIMcoinAmount = settings.value("nAnonymizeTIMcoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTIMcoinAmount"))
        SoftSetArg("-anonymizetimcoinamount", settings.value("nAnonymizeTIMcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in timcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTIMcoinAmount:
            return QVariant(nAnonymizeTIMcoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTIMcoinAmount:
            nAnonymizeTIMcoinAmount = value.toInt();
            settings.setValue("nAnonymizeTIMcoinAmount", nAnonymizeTIMcoinAmount);
            emit anonymizeTIMcoinAmountChanged(nAnonymizeTIMcoinAmount);
            break;
        case CoinControlFeatures:
