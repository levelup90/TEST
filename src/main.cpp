...
#if defined(NDEBUG)
#error "TIMcoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in utim) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zTIM is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zTIM spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zTIM without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zTIM spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zTIM spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zTIM spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-ztim");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zTIM mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	if (nHeight <= DynBlockStart || nHeight > DynBlockEnd || (!IsSporkActive(SPORK_21_CURRENT_DYNBLOCK_START) && !IsSporkActive(SPORK_22_CURRENT_DYNBLOCK_END)))
	{
		if (nHeight == 0) {
			nSubsidy = 40000000 * COIN;
		}
		else if (nHeight <= 59999 && nHeight > 0) {
			nSubsidy = 50 * COIN;
		}
		else if (nHeight <= 61000 && nHeight > 60000) {
			nSubsidy = 1 * COIN;
		}
		else if (nHeight <= 105000 && nHeight > 61000) {
			nSubsidy = 10 * COIN;
		}
		else if (nHeight <= 149000 && nHeight > 105000) {
			nSubsidy = 20 * COIN;
		}
		else if (nHeight <= 193000 && nHeight > 149000) {
			nSubsidy = 5 * COIN;
		}
		else if (nHeight <= 237000 && nHeight > 193000) {
			nSubsidy = 20 * COIN;
		}
		else if (nHeight <= 281000 && nHeight > 237000) {
			nSubsidy = 30 * COIN;
		}
		else if (nHeight <= 325000 && nHeight > 281000) {
			nSubsidy = 20 *
                }
                else if (nHeight <= 369000 && nHeight > 325000) {
                        nSubsidy = 10 * COIN;
                }
                else {
                        nSubsidy = 5 * COIN;
	...
	{
		Spork1Value = GetSporkValue(SPORK_19_CURRENT_BLOCK_1_COINSIZE);
		Spork2Value = GetSporkValue(SPORK_20_CURRENT_BLOCK_2_COINSIZE);
		CoinCheck = Spork1Value - Spork2Value;

		if ((Spork1Value == Spork2Value) && (CoinCheck == 0) && (Spork1Value <= SporkMax) && (Spork2Value <= SporkMax) && (Spork1Value >= SporkMin) && (Spork2Value >= SporkMin))
		{
				nSubsidy = Spork2Value * COIN;
		}
		else
		{
			nSubsidy = SporkMin * COIN;
		}
	}
	else
	{
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZTIMStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from TIMcoin
...
{
    RenameThread("timcoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZTIMMinted()
{
...
void RecalculateZTIMSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zTIM supply
...
bool RecalculateTIMSupply(int nHeightStart)
{
...
bool UpdateZTIMSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateTIMSupply(Params().Zerocoin_StartHeight());
    }

    //Track zTIM money supply in the block index
    if (!UpdateZTIMSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zTIM supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert TIM to zTIM
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zTIM spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zTIM serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zTIM stake block is less than allowed start height", __func__);

    if (CZPivStake* zTIM = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zTIM->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zTIM stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zTIM stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zTIM->GetValue()));
        if (nChecksum200 != zTIM->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zTIM->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZTIM() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zTIM fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zTIM mints and %d zTIM spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // TIMcoin: We use certain sporks during IBD, so check to see if they are
