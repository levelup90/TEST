...
public:
    ...
    bool WriteZTIMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZTIMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZTIMSeed_deprecated(uint256& seed);
    bool EraseZTIMSeed();
    bool EraseZTIMSeed_deprecated();

    bool WriteZTIMCount(const uint32_t& nCount);
    bool ReadZTIMCount(uint32_t& nCount);
